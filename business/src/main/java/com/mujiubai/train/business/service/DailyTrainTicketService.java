...
import com.mujiubai.train.business.domain.DailyTrainTicketExample;
import com.mujiubai.train.business.domain.TrainStation;
import com.mujiubai.train.business.enums.SeatTypeEnum;
import com.mujiubai.train.business.enums.TrainTypeEnum;
import com.mujiubai.train.business.mapper.DailyTrainTicketMapper;
import com.mujiubai.train.business.req.DailyTrainTicketQueryReq;
import com.mujiubai.train.business.req.DailyTrainTicketSaveReq;
...
public class DailyTrainTicketService {
    ...
    public PageResp<DailyTrainTicketQueryResp> queryList(DailyTrainTicketQueryReq req) {
        ...
        List<DailyTrainTicketQueryResp> list = BeanUtil.copyToList(dailyTrainTicketList,
    ...
    public void genDaily(DailyTrain dailyTrain, Date date, String trainCode) {
        ...
        for (int i = 0; i < stationList.size(); i++) {
            ...
            for (int j = (i + 1); j < stationList.size(); j++) {
                ...
                BigDecimal priceRate = EnumUtil.getFieldBy(TrainTypeEnum::getPriceRate, TrainTypeEnum::getCode,
                        trainType);
                BigDecimal ydzPrice = sumKM.multiply(SeatTypeEnum.YDZ.getPrice()).multiply(priceRate).setScale(2,
                        RoundingMode.HALF_UP);
                BigDecimal edzPrice = sumKM.multiply(SeatTypeEnum.EDZ.getPrice()).multiply(priceRate).setScale(2,
                        RoundingMode.HALF_UP);
                BigDecimal rwPrice = sumKM.multiply(SeatTypeEnum.RW.getPrice()).multiply(priceRate).setScale(2,
                        RoundingMode.HALF_UP);
                BigDecimal ywPrice = sumKM.multiply(SeatTypeEnum.YW.getPrice()).multiply(priceRate).setScale(2,
                        RoundingMode.HALF_UP);
    ...
    public DailyTrainTicket selectByUnique(Date date, String trainCode, String start, String end) {
        ...
        DailyTrainTicketExample.createCriteria().andDateEqualTo(date).andTrainCodeEqualTo(trainCode)
                .andStartEqualTo(start).andEndEqualTo(end);
